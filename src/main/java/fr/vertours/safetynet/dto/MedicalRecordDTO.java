...
//    }
    public static MedicalRecordDTO fromMedicalRecord(
        ...
        Set<String> allergySet = medicalRecord.getAllergies().stream()
                .map(Allergy::getAllergy).collect(Collectors.toSet());
        medicalRecordDTO.setAllergies(allergySet);
        Set<String> medicationSet = medicalRecord.getMedications().stream()
                .map(Medication::getMedication).collect(Collectors.toSet());
